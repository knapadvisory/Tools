...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/') {
      ...
      res.end('<!doctype html><meta charset="utf-8"><title>KNAP Tally Audit Engine</title>' +
        '<body style="font:15px system-ui;padding:40px;max-width:560px;margin:auto">' +
        '<h2>🔍 KNAP Tally Audit Engine v' + VERSION + '</h2>' +
        '<p>Running (managed by the KNAP Tally Connector). The tool lives at</p>' +
        '<p><a href="' + HUB + '/audit/">' + HUB + '/audit/</a></p>');
      return;
    }
...
if (process.argv.includes('--selftest')) {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.log('');
    console.log('  KNAP Tally Audit Engine v' + VERSION + ' running on 127.0.0.1:' + PORT + '.');
    console.log('  The tool page: ' + HUB + '/audit/');
