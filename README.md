...
| Debtor / Creditor Consolidation | **Hosted here** at `/debtor-creditor/` + local connector | This repo — page `debtor-creditor/`, engine in `connector/knap-tally-connector.mjs` (`/api/dc/*`, v4.3). Balances are Tally's own group closing (ties to Group Summary). Reports: consolidated matrix, 80/20 Pareto, party master; master workbook bundles all (ageing + bill-wise are next). |
