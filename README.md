...
| Debtor / Creditor Consolidation | **Hosted here** at `/debtor-creditor/` + local connector | This repo — page `debtor-creditor/`, engine in `connector/knap-tally-connector.mjs` (`/api/dc/*`, v4.8). Balances = master FY-start opening + voucher movement to the as-on date (ties to Tally's Group Summary). Reports: consolidated matrix, 80/20 Pareto, ageing, bill-wise summary, party master; master workbook bundles all. |
